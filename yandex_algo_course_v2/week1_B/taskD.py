...
# улицы по одну сторону от нее. По другую сторону от этой улицы пока ничего нет,
# но скоро все будет – школы, магазины, кинотеатры и т.д.

# Для начала в этой деревне решили построить школу.
# Место для строительства школы решили выбрать так,
# чтобы суммарное расстояние, которое проезжают ученики от своих домов до школы,
# было минимально.
...
# Формат вывода
# Выведите одно целое число — координату точки,
# в которой лучше всего построить школу. Если ответов несколько, выведите любой из них.
...
def tests():
    ...
    # # s = sum([abs(x) for x in vals]) / n
    # s = vals[n // 2]
    # s = round(s)
    # for s1 in [s - 50, s - 20, s - 2, s - 1, s, s + 1, s + 2, s + 20]:
    #     dist = sum([abs(s1 - x) for x in vals])
    #     print(s1, dist)
